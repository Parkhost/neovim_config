local function set_augroup()
  vim.api.nvim_command("augroup WrapInMarkdown")
  vim.api.nvim_command("autocmd!")
  vim.api.nvim_command("autocmd FileType markdown setlocal wrap")
  vim.api.nvim_command("augroup END")
  vim.api.nvim_command("augroup highlight_yank")
  vim.api.nvim_command("autocmd!")
  vim.api.nvim_command("autocmd TextYankPost * silent! lua require'vim.highlight'.on_yank({timeout = 240})")
  vim.api.nvim_command("augroup END")
end

local function set_vim_g()
  vim.g.mapleader = " "
end

local function set_vim_o()
  local settings = {
    backup = false,
    errorbells = false,
    expandtab = true,
    hidden = true,
    scrolloff = 3,
    softtabstop = 2,
    showmode = false,
    termguicolors = true,
		number = true
  }

  -- Generic vim.o
  for k, v in pairs(settings) do
    vim.o[k] = v
  end

  -- Custom vim.o
  vim.o.clipboard = 'unnamedplus'
  vim.o.shortmess = vim.o.shortmess .. 'c'

  -- Not yet in vim.o
  vim.cmd('set encoding=utf8')
  vim.cmd('set nowritebackup')
  vim.cmd('set shiftwidth=2')
  vim.cmd('set secure')
  vim.cmd('set splitright')
  vim.cmd('set tabstop=2')
	vim.cmd('set updatetime=300')
	vim.cmd('set nohlsearch')
end


local function set_vim_wo()
  vim.wo.number = true
  vim.wo.relativenumber = true
  vim.wo.wrap = false
end
local function set_keymaps()
  local map = vim.api.nvim_set_keymap

  local options = { noremap = false }

  map('n', '<leader>h', '<CMD>wincmd h<CR>', options)
  map('n', '<leader>j', '<CMD>wincmd j<CR>', options)
  map('n', '<leader>k', '<CMD>wincmd k<CR>', options)
  map('n', '<leader>l', '<CMD>wincmd l<CR>', options)

  -- keep search result centered
  map('n', 'n', 'nzzzv', options)
  map('n', 'N', 'Nzzzv', options)

  -- drag selected up or down ..?
  map('v', 'J', ':m \'>+1<CR>gv=gv', options)
  map('v', 'K', ':m \'<-2<CR>gv=gv', options)

  -- next greatest remap ever : asbjornHaland
  map('n', 'Y', 'y$', options)
  map('n', '<leader>y', '"+y', options)
  map('v', '<leader>y', '"+y', options)
  map('n', '<leader>Y', 'gg"+yG', options)

  map('n', '<leader>d', '"_d', options)
  map('v', '<leader>d', '"_d', options)

  --map("n", "<space>f", "<cmd>lua vim.lsp.buf.formatting()<CR>", options)

  -- coc format text?
  --map('x', '<leader>cf', '<Plug>(coc-format-selected)', options)
  --map('n', '<leader>cf', '<Plug>(coc-format-selected)', options)

end

local function init()
  set_augroup()
  set_vim_g()
  set_vim_o()
  set_vim_wo()
  set_keymaps()
end

return {
  init = init
}
